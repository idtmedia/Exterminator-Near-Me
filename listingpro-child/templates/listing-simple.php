					...
					if( !empty($_GET['s']) && isset($_GET['s']) && $_GET['s']=="home" ){
						...
						if( empty($_GET['lp_s_tag']) && empty($_GET['lp_s_cat']) && !empty($_GET['select']) ){
							...
							if( $lpDefaultSearchBy=="title" ){
								...
								//Add support search by Zip code						
								$doc = new DOMDocument();
								$url = 'http://maps.googleapis.com/maps/api/geocode/xml?address='.$sKeyword.'@&sensor=true';
								$doc->load($url);
								...
								if($city) $sKeyword = $city;	
								//End Add support search by Zip code								
								$defSquery = $sKeyword;								
							}else{
